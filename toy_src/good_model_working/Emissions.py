...
class EmissionHeatMap:
    ...
    def ask_emission_criteria(self):
        ...
        print("Available Emission Criteria:")
        for key in emission_options.keys():
            print(f"- {key}")

        emission_choice = input("Enter the emission criteria you'd like to map (e.g., CO2, NOx, SO2): ").strip()

        if emission_choice not in emission_options:
            print("Invalid emission criteria. Please enter a valid option (e.g., CO2, NOx).")
            return
    ...
    #
    # def plot_emissions(self, emission_type='PLCO2RTA', y_limit=None):
    #     emissions_per_mwh = self.calculate_region_emissions()
    #     if emission_type not in ['PLPMTRO', 'PLNOXRTA', 'PLSO2RTA', 'PLCO2RTA', 'PLCH4RTA', 'PLN2ORTA']:
    #         raise ValueError("Invalid emission type. Choose from: 'PLPMTRO', 'PLNOXRTA', 'PLSO2RTA', 'PLCO2RTA', 'PLCH4RTA', 'PLN2ORTA'")
    #
    #     regions = list(emissions_per_mwh.keys())
    #     emissions = [emissions_per_mwh[region][emission_type] for region in regions]
    #
    #     # Modify the emission type for label (remove first two and last three letters)
    #     emission_type_label = emission_type[2:-3]
    #
    #     fig, ax = plt.subplots(figsize=(14, 8))
    #
    #     bars = ax.bar(regions, emissions, color='red')
    #
    #     ax.set_xlabel('Regions', fontsize=18)  # Increased font size for X-axis
    #     ax.set_ylabel(f'{emission_type_label} Emissions (lbs/MWh)', fontsize=18)  # Larger Y-axis label
    #     ax.set_title(f'{emission_type_label} Emissions per Region', fontsize=20)  # Larger title
    #
    ...
    #     plt.xticks(rotation=45, ha='right', fontsize=10)  # Larger font size for X-axis labels
    #
    #     plt.tight_layout()
    #     plt.show()
    #
...
# %%
#
emission_heat_map = EmissionHeatMap()
emission_heat_map.load_data()
emission_heat_map.generation_county()
# emission_heat_map.ask_emission_criteria()
emission_heat_map.create_capacity_map(save_as_html=True, html_filename='capacity_map.html')
